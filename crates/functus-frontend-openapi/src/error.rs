...
pub enum FrontendError {
    ...
    /// フロントエンドが自動生成する対象名(`<operationId>Params` 等)が
    /// `components.schemas` の既存の対象名と衝突している。
    #[error(
        "操作 `{operation_id}` の入力をまとめるために生成する対象名 `{name}` が、\
         既存の components.schemas の対象名と衝突している。operationId を変更するか、\
         該当スキーマの名前を変更すること"
    )]
